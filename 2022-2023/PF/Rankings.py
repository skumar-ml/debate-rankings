...
ddTeams = []
with open("23-24-PFCT_Roster.csv", "r") as fp:
    for line in fp:
...
def write_to_csv(elosList):
    ...
    for team, eloSchool in elosList:
        ...
        if school in bidList:
            ...
            bids = 0
        if names in ddTeams:
            add += str(counter) + "," + school + "," + names + "," + str(round(elo * 1000) / 1000) + f",Y\n"
        else:
            add += str(counter) + "," + school + "," + names + "," + str(round(elo*1000)/1000) + f",N\n"
...
add_tournament("Peninsula", 2)
add_tournament("MLK", 4)
...
add_tournament("MillardNorth", 4)
add_tournament("Lakeland", 1)
add_tournament("Houston", 1)
