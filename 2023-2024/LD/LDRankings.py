...
ddTeams = []
with open("23-24-LDCT-Roster.csv", "r") as fp:
    for line in fp:
...
def write_to_csv(elosList):
    ...
    for team, eloSchool in elosList:
        ...
        if name in bidList:
            ...
            bids = 0
        if (name in ddTeams) and ("Andrew Park" not in name):
        ...
        if name in nsdTeams and ("Andrew Park" not in name):
            add += ",Y\n"
        else:
            add += ",N\n"
...
add_tournament("FloridaBlueKey", 4)
...
add_tournament("UHouston", 2) # Occuring Jan 5 weekend
add_tournament("Churchill", 2)
add_tournament("Arizona", 1)
...
add_tournament("UNLV", 2)
add_tournament("UPenn", 1)
add_tournament("UpperStClair", 1)
add_tournament("Stanford", 4)
add_tournament("Berkeley", 8)
add_tournament("Harvard", 8)
add_tournament("MilliardNorth", 1) # Happening 2/23 weekend
add_tournament("TOCDigital2", 4)
add_tournament("TOCDigital3", 4)
add_tournament("TOC", 16)


# add_tournament("HolyCross", 2) # Cancelled
# add_tournament("Duke", 1) # Cannot find on Tab
# add_tournament("Ridge", 1) # No results on tab
# add_tournament("Colleyville") # Cannot find on Tab
